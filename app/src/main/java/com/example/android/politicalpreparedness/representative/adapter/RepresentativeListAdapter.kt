...
class RepresentativeListAdapter :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RepresentativeViewHolder {
        val item = ItemRepresentativeBinding.inflate(LayoutInflater.from(parent.context));
        return RepresentativeViewHolder(item)
    }
...
class RepresentativeViewHolder(val binding: ItemRepresentativeBinding) :
    ...
    fun bind(item: Representative) {
        ...
        item.official.channels?.apply { showSocialLinks(this) }
        item.official.urls?.apply { showWWWLinks(this) }
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!twitterUrl.isNullOrBlank()) {
            enableLink(binding.xIcon, twitterUrl)
        }
    }
